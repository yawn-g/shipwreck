...
void init_game() {
  ...
  Serial.println(); Serial.println("=== NEW GAME ==="); Serial.println();
  byte v = 0;
        ...
        // if p2 is CPU
          ...
          for (byte y = 0; y < BOARD_SIZE; y++) {
            for (byte x = 0; x < BOARD_SIZE; x++) {
              ...
              // if shot successful
              byte scanned_shot = shots[CPU][x][y];
              if (scanned_shot < 5) {

                // if boat not sunk
                if (!sunk(HUMAN, scanned_shot)) {

                  // check neighbouring cells for connected hits
                  bool axis;
                  bool axis_found = false;
                  bool available_neighbour = false;
                  Serial.print("Checking neighbours for "); Serial.print(x); Serial.print(", "); Serial.println(y);
                  for (byte i = 0; i < 2; i++) {
                    for (byte j = 0; j < 2; j++) {
                      
                      // translate i, j to coordinates of corresponding neighbour depending on axis
                      ...
                      Serial.print("  Neighbour: "); Serial.print(neighbour_x); Serial.print(", "); Serial.print(neighbour_y); Serial.print(" is ");
  
                      // if coordinates of neighbour within board
                      if (neighbour_x >= 0 && neighbour_x < BOARD_SIZE && neighbour_y >= 0 && neighbour_y < BOARD_SIZE) {
                        ...
                        if (check < 6 && !sunk(HUMAN, check)) {
                          if (neighbour_x == x) {                      
                            Serial.println("  2 vertically connected hits on floating boats!");
                            axis = VERTICAL;
                            axis_found = true;
                          } else {
                            Serial.println("  2 horizontally connected hits on floating boats!");
                            axis = HORIZONTAL;
                            axis_found = true;
                          }
                        } // end if hit & not sunk
                          ...
                          available_neighbour = true;
                        }
                      } // end if neighbour coordinates within board
                  ...
                  if (axis_found) {
                    ...
                    Serial.print("  Axis found: "); Serial.println(axis);
                    // go towards both ends
                    for (byte k = 0; k < 2; k++) {
                                            
                      // set stop & step according to direction
                      int start, stop_, step_;
                      if (k == 0) {
                        start = 1;
                        step_ = 1;
                        stop_ = 5;
                      ...
                      // look for spot to shoot at
                      for (int offset = start; offset != stop_; offset += step_) {
                        ...
                        if (axis == HORIZONTAL) {
                          ...
                          int spot_x = x + offset;
                          byte spot_result;
                          Serial.print(spot_x); Serial.print(", "); Serial.print(y); Serial.print(": ");
                                 
                          // if step is within board
                          if (spot_x >= 0 && spot_x < BOARD_SIZE) {
                                    
                            // shoot if empty
                            spot_result = shots[CPU][spot_x][y];
                            Serial.println(spot_result);
                            if (spot_result == 255) {
                              // write shot to map
                              write_CPU_shot(spot_x, y);
                              // don't go further!!!
                              shot_set = true;
                              break;
                            ...
                            // if a boat is met in spot
                            } else if (spot_result < 5) {
                              Serial.print("  Already shot there: "); Serial.print(spot_result); Serial.println(", continue scanning");
                            }
                          }
                        }
                        ...
                        // vertically
                          ...
                          if (spot_y >= 0 && spot_y < BOARD_SIZE) {
                            ...
                            // shoot if empty
                            spot_result = shots[CPU][x][spot_y];
                            Serial.println(spot_result);
                            if (spot_result == 255) {
                              // write shot to map
                              write_CPU_shot(x, spot_y);
                              // don't go further!!!
                              shot_set = true;
                              break;
                              // if already a missed shot there
                        ...
                        } // end if vertical scan
                        if (shot_set) break;
                      } // end scan
                      if (shot_set) break;
                    } // end for each side of axis                    
                  } // end if axis found

                  // if no 2 connected hits and a neighbour is available
                  else if (available_neighbour) {

                    Serial.println("  No axis found. Aiming @ rnd neighb.");
                    // aim at random neighbour
                    bool aiming = true;
                    while (aiming) {
                      int aim_x = x;
                      int aim_y = y;
                      Serial.print("  Aiming (");
                      bool shooting_axis = random(2);
                      bool shooting_cell = random(2);
                      Serial.print(shooting_axis); Serial.print(", "); Serial.print(shooting_cell); Serial.print(")");
                      // if horizontal axis picked
                      if (shooting_axis == HORIZONTAL) {
                        if (shooting_cell) aim_x++;
                        else aim_x--;
                      } else {
                        if (shooting_cell) aim_y++;
                        else aim_y--;
                      }

                      if (aim_x >= 0 && aim_x < BOARD_SIZE && aim_y >= 0 && aim_y < BOARD_SIZE) {
                        if (shots[CPU][aim_x][aim_y] == 255) {
                          Serial.println();
                          write_CPU_shot(aim_x, aim_y);
                          shot_set = true;
                          aiming = false;
                        }
                        else {
                      ...
                      } // end if aim is within board
                    } // end while aiming
                  } // end shooting at neighbour
                } // end if parent not sunk
              } // end if parent successful
              if (shot_set) break;
            } // end for each CPU shot (x)
            if (shot_set) break;
          } // end for each CPU shot (y)

          // if no previous successful shot to investigate
          if (!shot_set) {
            bool aiming = true;
            while (aiming) {
